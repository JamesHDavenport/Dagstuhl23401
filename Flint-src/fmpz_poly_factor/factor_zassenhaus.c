...
/* #define TRACE_ZASSENHAUS 1 moved to header */
...
/*
    ...
    is due to Mignotte, see e.g., Cohen Theorem 3.5.1 p.\ 133.
               ...
               slong exp, const fmpz_poly_t f, slong cutoff, int use_van_hoeij)
{
    ...
    else
    {
        ...
        for (i = 0; i < 7; i++) /* JHDraises to 5 */
        {
			...
			#if TRACE_ZASSENHAUS >= 1
                        flint_printf("|degree factorization mod p, p = %wd:\n f = ", p);
                        for(slong i=0; i < temp_fac->num; ++i)
                            flint_printf("%d^%d ", temp_fac->p[i].length - 1, temp_fac->exp[i]);
                        flint_printf("\n");
			flint_printf("|possible_degs after factoring mod p, p = %wd:\n", p);
                        for(slong i=0; i <= Z->deg; ++i) {
                            if( Z->pos_degs[i] == 1 ) {
                                flint_printf("%d ", i);
                            }
                        }
