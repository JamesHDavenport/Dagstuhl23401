...
lemma Mignotte1974L1a (p :  ℂ[X])  (α β: ℂ) : (L2normSq ((X+ Polynomial.C α)*p):ℂ) =  ( ∑ᶠ l:ℕ,  (( ((‖p.coeff (l)‖^2:ℝ)) +  α*p.coeff (l+1)*conj (p.coeff (l)) +conj ( α)*(p.coeff (l))*conj (p.coeff (l+1))+((‖α * p.coeff (l+1)‖^2:ℝ))).re):ℝ)  := by
